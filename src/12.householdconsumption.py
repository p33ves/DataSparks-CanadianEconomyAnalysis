...
def main():
    ...
    exp_notnull_df = exp_df.filter(
        exp_df['REF_DATE'].isNotNull() | exp_df['GEO'].isNotNull() | exp_df['Estimates'].isNotNull() | exp_df[
            'VALUE'].isNotNull())

    # fetch data for the last 10 years
    exp_decade_df = exp_notnull_df.where(
        exp_notnull_df['REF_DATE'].between(datetime.datetime.strptime('2010-01-01', '%Y-%m-%d'),
                                           datetime.datetime.strptime('2020-10-01', '%Y-%m-%d')))

    # Fecting only "Seasonally adjusted at quarterly rates values
    exp_seasonal_df = exp_decade_df.filter(
        exp_decade_df['Seasonal adjustment'] == lit('Seasonally adjusted at quarterly rates'))
    ...
    exp_int_df = exp_date.withColumn('household_expenditure',
                                     (exp_date['household_expenditure']).cast(types.IntegerType()))
