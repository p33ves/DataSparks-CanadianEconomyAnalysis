...
def main():
	...
	gdp_avg = gdp_int_df.groupBy(year('REF_DATE').alias('YEAR')).agg(round(avg(gdp_int_df['GDP_VALUE']),2).alias('RETAIL_GDP*(10^3)'))
	...
	#fetch data for the last 10 years
	trade_decade_df = trade_date_df.where(trade_date_df['REF_DATE'].between(datetime.datetime.strptime('2010-01-01', '%Y-%m-%d'), datetime.datetime.strptime('2020-10-01','%Y-%m-%d')))
	...
	trade_date_df = trade_date_df.filter(trade_date_df['North American Industry Classification System (NAICS)'] == lit('Retail trade [44-45]'))
	...
	#convert 'GDP_VALUE' to int type
	trade_int_df = trade_sales.withColumn('RETAIL_TRADE_SALES', (trade_sales['RETAIL_TRADE_SALES']).cast(types.IntegerType()))

	#Get yearly average data for Retail Trade GDP
	trade_avg = trade_int_df.groupBy(year('REF_DATE').alias('YEAR')).agg(avg(trade_int_df['RETAIL_TRADE_SALES']).alias('RETAIL_TRADE_SALES *(10^6)'))
